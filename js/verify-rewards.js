...
{
  fresh(103);
  // cup 1: races bank 15 (three legs), player abandons — no settle
  M.addXp(15);
  // cup 2 completes: 5 more legs' worth + completion + points
  M.addXp(5 + 10 + 40);                            // total 70 -> level 2 (50)
...
// E: the xp card snapshot, built the flow.js way ---------------------------------------
{
  ...
  M.addXp(X.XP_RACE * 4);                          // four legs
  M.addXp(X.XP_CUP + X.XP_PER_POINT * 26);         // completion, median points
  const to = M.pilotXp();
  M.queueReward({ kind: 'xp', from, to, added: to - from,
    levelFrom: X.levelFor(from), levelTo: X.levelFor(to) });
  const e = M.pendingRewards()[0];
  check('E xp card snapshot: 56 added, level 1 -> 2',
    e.added === 56 && e.from === 0 && e.to === 56
