...
//   * XP_CUP for completing all four legs — matching the cup's own
//     rule that an attempt only counts when whole.
//   * XP_PER_POINT per cup point — your points BECOME xp, a law the
//     player can verify in their head. Points are the skill signal
//     (13-minus-place per race), so a completed cup pays 34 to a
//     back-marker and 78 to a sweep: a 2.3x spread, enough for skill
//     to matter without punishing the slow.
//
...
const XP_RACE = 5;        // per race finished
const XP_CUP = 10;        // per cup completed (all four legs)
...
// pilot crossed the line in; completed means all four legs were RUN
// (the cup's own definition); points is the cup total, which only
